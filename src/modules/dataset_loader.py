...
async def load_dataset(config: Dict[str, Any], source_name: str) -> None:
    """
    Load data from a dataset configuration.
    ...
    files_to_process = get_files_to_process(config, folder_path)
...
async def load_all_datasets(sources: Dict[str, list]) -> None:
    ...
    """
    dataset_configs = sources.get("datasets", [])
    ...
    print(f"\n{'='*60}")
    print(f"Loading {len(dataset_configs)} dataset source(s)")
    print(f"{'='*60}")
    
    for dataset_config in dataset_configs:
        source_name = dataset_config.get("_source_folder", "unknown")
        try:
            await load_dataset(dataset_config, source_name)
        except Exception as e:
